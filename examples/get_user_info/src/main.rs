...
use openfrp_sdk::{login, get_user_info}; // Import login and get user info APIs 导入 登录 和 获取用户信息 的API
...
fn main() -> Result<()> {
    tokio::runtime::Builder::new_current_thread() // Create a single threads runtime with tokio 用tokio创建一个单线程的异步运行时环境
        ...
        .block_on(async { // Blocks current thread until tasks completed 阻塞当前线程，直到任务完成
            ...
            let user_info = get_user_info::get_user_info(&auth, client).await?; // Get user info 获取用户信息
            println!("{user_info:#?}"); // Print it 打印它
            Ok(())
        })
}
