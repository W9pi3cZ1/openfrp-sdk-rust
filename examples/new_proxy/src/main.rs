...
use openfrp_sdk::{login, new_proxy}; // Import login and new proxy APIs 导入 登录 和 新建隧道 的API
...
fn main() -> Result<()> {
    tokio::runtime::Builder::new_current_thread() // Create a single threads runtime with tokio 用tokio创建一个单线程的异步运行时环境
        ...
        .block_on(async {
            ...
            let new_proxy = new_proxy::new_proxy(&auth, &proxy, client).await?;
            println!("{new_proxy:#?}"); // Print request result 打印请求结果
            Ok(())
        })
}
