...
use openfrp_sdk::{login, get_node_list}; // Import login and get node list APIs 导入 登录 和 获取节点列表 的API
...
fn main() -> Result<()> {
    tokio::runtime::Builder::new_current_thread() // Create a single threads runtime with tokio 用tokio创建一个单线程的异步运行时环境
        ...
        .block_on(async { // Blocks current thread until tasks completed 阻塞当前线程，直到任务完成
            ...
            let node_list = get_node_list::get_node_list(&auth, client).await?; // Get node list 获取节点列表
            println!("{node_list:#?}"); // Print it 打印它
            Ok(())
        })
}
